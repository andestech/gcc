...
nds32_expand_extv (rtx *operands)
{
  ...
  if (MEM_P (src)
    ...
    {
      rtx newmem, base_addr;

      if (BYTES_BIG_ENDIAN)
	bitpos = GET_MODE_BITSIZE (GET_MODE (dst)) - width - bitpos;

      if (width == 32)
	{
	  newmem = adjust_address (src, SImode,
				   bitpos / BITS_PER_UNIT);
	  base_addr = force_reg (Pmode, XEXP (newmem, 0));
      ...
      else
	{
	  ...
	  if (GET_CODE (dst) == SUBREG
	      && GET_MODE (dst) == SImode
	      && GET_MODE (SUBREG_REG (dst)) == HImode)
	    dst = SUBREG_REG (dst);

	  if (GET_MODE_BITSIZE (GET_MODE (dst)) != width)
	    return EXPAND_FAIL;

	  newmem = adjust_address (src, HImode,
				   bitpos / BITS_PER_UNIT);
	  base_addr = force_reg (Pmode, XEXP (newmem, 0));
	  emit_insn (gen_unaligned_loadhi (dst, base_addr));
	}
...
nds32_expand_insv (rtx *operands)
{
  ...
  if (MEM_P (dst)
      ...
      && (bitpos % BITS_PER_UNIT)  == 0)
    {
      ...
      if (BYTES_BIG_ENDIAN)
	bitpos = GET_MODE_BITSIZE (GET_MODE (src)) - width - bitpos;

      if (width == 32)
	{
	  newmem = adjust_address (dst, SImode,
				   bitpos / BITS_PER_UNIT);
	  base_addr = force_reg (Pmode, XEXP (newmem, 0));
      ...
      else
	{
	  ...
	  newmem = adjust_address (dst, HImode,
				   bitpos / BITS_PER_UNIT);
	  base_addr = force_reg (Pmode, XEXP (newmem, 0));
	  emit_move_insn (tmp, gen_lowpart (HImode, src));
...
nds32_expand_unaligned_load (rtx *operands, enum machine_mode mode)
{
  ...
  for (num_reg = (mode == DImode) ? 2 : 1; num_reg > 0; num_reg--)
      ...
      while (width > 0)
	{
	  ...
	  emit_insn (gen_zero_extendqisi2 (temp_reg, temp_sub_reg));
...
nds32_spilt_doubleword (rtx *operands, bool load_p)
{
  ...
  else if (GET_CODE (sub_mem) == POST_DEC)
    {
      /* memory format is (post_dec (reg)),
	 so that extract (reg) from the (post_dec (reg)) pattern.  */
  ...
  if ((load_p && reg_overlap_mentioned_p (low_part[0], high_part[1]))
      || GET_CODE (sub_mem) == POST_MODIFY || GET_CODE (sub_mem) == POST_DEC)
    {
...
rtx nds32_di_high_part_subreg(rtx reg)
{
  ...
  ret = simplify_gen_subreg (SImode, reg,
			     DImode, high_part_offset);
  if (!ret)
    {
      tmp = gen_reg_rtx (DImode);
      emit_move_insn (tmp, reg);
      return simplify_gen_subreg (SImode, tmp,
				  DImode, high_part_offset);
    }
}
