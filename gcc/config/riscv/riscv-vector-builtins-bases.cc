...
public:
  rtx expand (function_expander &e) const override
  {
    ...
    if (TARGET_BF16 && e.op_info->op == OP_TYPE_bf_v)
      {
	e.add_input_operand (0);
	return e.generate_insn (code_for_vfwcvt_bf16 (e.vector_mode ()));
      }
    if (TARGET_ZVFBFMIN && e.op_info->op == OP_TYPE_bf_v)
      return e.use_exact_insn (code_for_pred_extend_bf (e.vector_mode ()));
    gcc_unreachable ();
  }
};
...
template<enum frm_op_type FRM_OP = NO_FRM>
class vfwmaccbf : public function_base
{
public:
  bool has_rounding_mode_operand_p () const override
  {
  ...
  rtx expand (function_expander &e) const override
  {
    if (e.op_info->op == OP_TYPE_vf)
      return e.use_widen_ternop_insn (
	code_for_pred_widen_mul_plus_bf_scalar (e.vector_mode ()));
    if (e.op_info->op == OP_TYPE_vv)
      return e.use_widen_ternop_insn (
	code_for_pred_widen_mul_plus_bf (e.vector_mode ()));
    gcc_unreachable ();
  }
};

template<enum frm_op_type FRM_OP = NO_FRM>
class vfncvtbf16_bf : public function_base
{
public:
  bool has_rounding_mode_operand_p () const override
  {
  ...
  rtx expand (function_expander &e) const override
  {
    if (TARGET_ZVFBFMIN && e.op_info->op == OP_TYPE_f_w)
      return e.use_exact_insn (code_for_pred_trunc_bf16 (e.vector_mode ()));
    else if (TARGET_BF16 && e.op_info->op == OP_TYPE_f_w)
      {
	e.add_input_operand (0);
	return e.generate_insn (code_for_vfncvt_bf16 (e.vector_mode ()));
      }
    gcc_unreachable ();
...
static CONSTEXPR const vfwmaccbf<NO_FRM> vfwmaccbf16_obj;
static CONSTEXPR const vfwmaccbf<HAS_FRM> vfwmaccbf16_frm_obj;
static CONSTEXPR const vfwcvt_f vfwcvtbf16_f_obj;
static CONSTEXPR const vfncvtbf16_bf<NO_FRM> vfncvtbf16_bf_obj;
static CONSTEXPR const vfncvtbf16_bf<HAS_FRM> vfncvtbf16_bf_frm_obj;
