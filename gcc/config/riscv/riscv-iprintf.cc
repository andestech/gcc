...
iprintf_dom_walker::handle_gimple_call (gimple *gs)
{
  ...
  /* If alternate 'asm' name is given, don't change it */
  if (DECL_ASSEMBLER_NAME (fndecl) != DECL_NAME (fndecl))
    return NULL;
  ...
  id = DECL_NAME (new_fndecl) = get_identifier (new_fnname);
  SET_DECL_ASSEMBLER_NAME (new_fndecl, id);
  return new_fndecl;
}
